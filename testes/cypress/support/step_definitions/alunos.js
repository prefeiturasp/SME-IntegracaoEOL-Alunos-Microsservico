...
When("realizo consulta de acompanhamento escolar", () => {
  cy.apiGet(`/api/v1/alunos/dados-acompanhamento-escolar`).as("response");
});
...
When("realizo consulta de matrículas", () => {
  cy.apiGet(`/api/v1/alunos/matriculas`).as("response");
});

When("realizo consulta de matrículas de anos anteriores", () => {
  cy.apiGet(`/api/v1/alunos/matriculas/anos-anteriores`).as("response");
});

Then("o status da resposta de alunos deve ser válido", () => {
  cy.get("@response").then((response) => {
    expect([200, 201, 202, 204, 400, 404]).to.include(response.status);
  });
});
